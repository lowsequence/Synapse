...
class SnipeView(discord.ui.View):
    ...
    async def send_snipe_embed(self, interaction: discord.Interaction):
        ...
        embed.description = (
            f"**Deleted by:** <@{snipe['author_id']}>\n"
            f"**Deleted:** <t:{snipe['deleted_at']}:R>\n"
        )

        if snipe['content']:
            embed.add_field(name=" **Deleted Content:**", value=snipe['content'])
        if snipe['attachments']:
            attachment_links = "\n".join([f"[{attachment['name']}]({attachment['url']})" for attachment in snipe['attachments']])
            embed.add_field(name="**Deleted Attachments:**", value=attachment_links)
    ...
    @discord.ui.button(emoji="<:DoubleLeft:1469267782184206442>", style=discord.ButtonStyle.secondary, custom_id="first")
    async def first_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        self.index = 0
        self.update_buttons()
        await self.send_snipe_embed(interaction)

    @discord.ui.button(emoji="<:LeftArrow:1469267806150463488>", style=discord.ButtonStyle.secondary, custom_id="previous")
    ...
    @discord.ui.button(emoji="<:close:1469267685714956290>", style=discord.ButtonStyle.danger, custom_id="delete")
    async def delete_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.message.delete()

    @discord.ui.button(emoji="<:rightarrow:1469267754409529394>", style=discord.ButtonStyle.secondary, custom_id="next")
    ...
    @discord.ui.button(emoji="<:doubleright:1469267725103927316>", style=discord.ButtonStyle.secondary, custom_id="last")
...
class Snipe(commands.Cog):
    ...
    async def snipe(self, ctx):
        ...
        if first_snipe['content']:
            embed.add_field(name=f"**Deleted By:** <@{first_snipe['author_id']}>\n**Deleted Content:**", value=first_snipe['content'])
